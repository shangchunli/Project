...
// 产后锻炼页面渲染
router.post('/findaexercise',function(req,res,next){
  var userid = req.body.userid;
  var title = req.body.title;
  console.log(userid,title);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select chapterid,title,content,owner,time  from chapters where title=? AND tab="产后锻炼"',[title],function(err,result){
    if(err){
      console.log(err)
    }else{
      res.send(result);
...
// 产前锻炼页面渲染
router.post('/findbexercise',function(req,res,next){
  var userid = req.body.userid;
  var title = req.body.title;
  console.log(userid,title);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select chapterid,title,content,owner,time  from chapters where title=? AND tab="产前锻炼"',[title],function(err,result){
    if(err){
      console.log(err)
    }else{
      res.send(result);
...
router.post('/addhistory',function(req,res,next){
  var history1 = req.body.history;
  var userid = req.body.userid;  
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query("insert into history(history,userid) value(?,?)",[history1,userid],function(err,result){
    if(err){
      console.log(err);
    }else{
      console.log(result);
...
// 搜索历史返回
router.post('/findhistory',function(req,res,next){
  
  var userid = req.body.userid;
  console.log(userid);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select history from history where userid=?',[userid],function(err,result){
    if(err){
      console.log(err)
    }else{
      res.send(result);
...
// 返回搜索数据
router.post('/findhistoryresult',function(req,res,next){
  
  var history = req.body.history;
  // console.log(userid);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select chapterid,title,content,owner,time  from chapters  ',function(err,result){
    if(err){
      console.log(err)
    }else{
      res.send(result);
...
//我的相册
router.post('/xiangce',function(req,res,next){
  var powner=req.body.userId;
  var head = req.body.files[0].url;
  var timer =  Date.now() ;
  var imgpath = './server/head/'+ powner+'.png'; 
  var base64 = head.replace(/^data:image\/\w+;base64,/, "");//去掉图片base64码前面部分data:image/png;base64
  var dataBuffer = new Buffer(base64, 'base64'); //把base64码转成buffer对象，
  console.log('dataBuffer是否是Buffer对象：'+Buffer.isBuffer(dataBuffer));
  ...
  var imagepath = 'http://localhost:5001/getheadImg/?imgId='+powner;
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query("insert into picture(powner,pimage) values(?,?)",
        [powner,imagepath],function(err,result){
    if(err){
      console.log(err);
    }else{
      console.log(result);
...
router.post('/mypicture',function(req,res,next){
  ...
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select powner,pimage from picture where powner =? ',powner,function(err,result){
    if(err){
      console.log(err);
    }else{
      console.log(result);
...
router.post('/addcaprice',function(req,res,next){
  ...
  +new Date().getHours()+":"+new Date().getMinutes();
  // console.log(cowner,ccontent,cimage,ctime);
  var timer = Date.now() ;
...
//帖子详情页收藏保持,获取当前的用户收藏的帖子
router.post('/getscbycuser',function (req,res,next) { 
  var userId=req.body.user;
  console.log(userId);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select chapterid from mycollect where telphone=?',[userId],function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send(result)
 ...
 //帖子详情页收藏取消
router.post('/unclloection',function (req,res,next) { 
  var userId=req.body.userId;
  var chapterId=req.body.chapterId;
  console.log(userId);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('delete from mycollect where telphone=? AND chapterid=?',[userId,chapterId],function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send('取消收藏成功')
  ...
  //帖子详情页收藏数减1
router.post('/lesssccount',function (req,res,next) { 
  var sccount=req.body.sccount - 1;
  var chapterId=req.body.chapterId;
  console.log(sccount,chapterId);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('update chapters set sccount =? where chapterid=?',[sccount,chapterId],function(err,result){
    if(err){
...
//在点赞表中添加点赞的相关信息：
router.post('/addazinfo', function(req,res,next){
  var chapterId = req.body.chapterId;
  var userId = req.body.userId;
  // console.log("user:"+user,"chapterId:"+chapterId,'dzcount'+dzcount);
  var con  = mysql.createConnection(dbconfig);
  con.connect();
  con.query("insert into dz(telphone,chapterid) values(?,?)",[userId,chapterId],function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send(' adddzinfo success')
    }
  })
})

//在点赞表中删除点赞的相关信息：
router.post('/lessdzinfo', function(req,res,next){
  var chapterId = req.body.chapterId;
  var userId = req.body.userId;
  // console.log("user:"+user,"chapterId:"+chapterId,'dzcount'+dzcount);
  var con  = mysql.createConnection(dbconfig);
  con.connect();
  con.query("delete from dz where telphone=? AND chapterid=?",[userId,chapterId],function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send(' adddzinfo success')
...
//帖子详情页点赞保持,获取当前的用户点赞的帖子
router.post('/getdzbycuser',function (req,res,next) { 
  var userId=req.body.user;
  console.log(userId);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select chapterid from dz where telphone=?',[userId],function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send(result)
...
//点赞数目-1：
router.post('/lessdzcounnt', function(req,res,next){
  var chapterId = req.body.chapterId;
  var dzcount = req.body.dzcount-1;
  // console.log("user:"+user,"chapterId:"+chapterId,'dzcount'+dzcount);
  var con  = mysql.createConnection(dbconfig);
...
router.post('/addtopic',function(req,res,next){
  ...
  console.log(userId,tcontent,ttime,headimg);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('insert into topic(userid,tcontent,ttime,headimg) values(?,?,?,?)',[userId,tcontent,ttime,headimg],function(err,result){
    if(err){
      console.log(err);
    }else{
      console.log(result);
...
router.post('/topic',function(req,res,next){
  ...
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select topicid,userid,tcontent,ttime,headimg from  topic ',function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send(result);
...
//话题详情页渲染
router.post('/topicxiangqing',function(req,res,next){
  var topicid = req.body.topicid;
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select topicid,userid,tcontent,headimg from topic where topicid=?',topicid,function(err,result){
    if(err){
      console.log(err);
    }else{
      // console.log(result);
...
//增加话题评论
router.post('/addtcomment', function(req,res,next){
  var user = req.body.userId;
  var topicid = req.body.topicid;
  var plcontent = req.body.plcontent;
  var pltime = new Date().getFullYear()+'/'+(new Date().getMonth()+1)+'/'+new Date().getDate()+"  "
  +new Date().getHours()+":"+new Date().getMinutes();
  console.log("user:"+user,"topicid:"+topicid);
  var con  = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select head from register where telphone=?',[user],function(err,result){
    if(err){
      ...
      console.log(result[0].head)
      con.query("insert into tcomment(telphone,topicid,plcontent,pltime,head) values(?,?,?,?,?)",[user,topicid,plcontent,pltime,result[0].head],function(err,result){
        if(err){
          console.log(err);
        }else{
          res.send('tcomment success')
        }
      })
    }
  })
...
//话题评论渲染
router.post('/showtcomment',function(req,res,next){
  var topicid = req.body.topicid;
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select telphone,plcontent,plid,pltime,head from tcomment where topicid=?',[topicid],function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send(result)
...
router.post('/addfeedback',function(req,res,next){
  ...
  var contact = req.body.contactDetails;
  console.log(userid,feedback,contact);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('insert into feedback(userid,feedback,contact) values(?,?,?)',[userid,feedback,contact],function(err,result){
    if(err){
      console.log(err);
    }else{
      res.send(result)
...
router.get('/showcomment',function(req,res,next){
  ...
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query("select * from comment where chapterid =?",[id],function(err,result){
    if(err){
      console.log(err)
    }else{
      // res.end("delete success");
      res.render('comment',{usersList:result});
...
//查找评论
router.post("/plsearch",function(req,res,next){
...
//删除评论
router.get('/delpl',function(req,res,next){
  var id = req.query.id;
  var tzid=req.query.tzid;
  console.log(tzid);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('delete from comment where plid=?',[id],function(err,result){
    if(err){
      console.log(err);
    }else{
      console.log('刪除成功')
      res.redirect('/showcomment/?id='+tzid)
    }
  })
})
//系统管理
...
//话题圈管理
//话题管理-渲染
router.get('/topicm',function(req,res,next){
  console.log('topics');
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('select * from  topic',function(err,result){
    if(err){
      ...
      res.render('topic',{usersList:result});
    }
  })
})
//话题删除
router.get('/deltopic',function(req,res,next){
  ...
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query("delete from topic where topicid =?",[id],function(err,result){
    if(err){
      console.log(err)
    }else{
      // res.end("delete success");
...
//话题讨论
router.get('/showtc',function(req,res,next){
  var id = req.query.id;
  console.log(id);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query("select * from tcomment where topicid =?",[id],function(err,result){
    if(err){
      console.log(err)
    }else{
      // res.end("delete success");
      res.render('tcomment',{usersList:result});
...
//查找评论
router.post("/tcsearch",function(req,res,next){
  var username = req.body.username;
  console.log(username);
  var sql = "select * from tcomment";
  if(username){
      sql += " where telphone = '"+ username +"'";
  }
  var con = mysql.createConnection(dbconfig);
...
//删除评论
router.get('/deltc',function(req,res,next){
  
  console.log('话题评论删除')
  var id = req.query.id;
  var htid= req.query.htid;
  console.log('话题id'+htid);
  console.log(id);
  var con = mysql.createConnection(dbconfig);
  con.connect();
  con.query('delete from tcomment where plid=?',[id],function(err,result){
    if(err){
      console.log(err);
    }else{
      console.log('刪除成功')
      res.redirect("/showtc/?id="+htid);
    }
  })
})
